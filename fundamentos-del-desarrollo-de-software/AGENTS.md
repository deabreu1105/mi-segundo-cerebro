...
- **Enlaces a fuentes procesadas DEBEN usar la ruta cualificada (`[[wiki/sources/nombre-fuente]]`)** — Para evitar que Obsidian resuelva por error hacia el archivo original en `raw/`, todo enlace a un resumen procesado en `index.md`, `log.md`, conceptos o entidades DEBE incluir la subcarpeta `wiki/sources/`.
- **Preserva explicaciones técnicas y desgloses completos** — La síntesis en `wiki/sources/` DEBE incluir la sección `## Detailed Breakdown` para conservar explicaciones conceptuales, sub-secciones, niveles de diagnóstico y metodologías originales sin simplificar en exceso el conocimiento.
- **Preserva diagramas y código en `wiki/sources/`** — Si la fuente en `raw/` incluye diagramas Mermaid (` ```mermaid `) o pseudocódigo/código, deben extraerse e incluirse íntegramente en las secciones `## Diagrams & Visualizations` y `## Code & Pseudocode Examples`.
