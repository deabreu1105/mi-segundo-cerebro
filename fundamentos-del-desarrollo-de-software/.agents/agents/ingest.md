...
- Toma nota mental de:
  ...
  - Diagramas gráficos (Mermaid ` ```mermaid `) presentes en la fuente
  - Bloques de código o pseudocódigo ilustrativos
...
**Secciones obligatorias y condicionales:**
...
- `## Detailed Breakdown` — **(Obligatoria)** Desglose detallado por secciones conservando las explicaciones conceptuales, sub-temas, definiciones, niveles de diagnóstico y metodologías expuestas en el `raw/`
- `## Diagrams & Visualizations` — *(Si la fuente los contiene)* Preservar y renderizar bloques Mermaid (` ```mermaid `) originales
- `## Code & Pseudocode Examples` — *(Si la fuente los contiene)* Preservar fragmentos clave de código o pseudocódigo
...
Añade una entrada en la sección `## 📑 Fuentes Procesadas` usando la ruta cualificada:
```
- [[wiki/sources/YYYY-MM-DD-titulo-fuente|YYYY-MM-DD-titulo-fuente]] — descripción de una línea
```
...
- ✅ **Enlaces Cualificados a Fuentes:** Enlazar siempre usando `[[wiki/sources/nombre-fuente]]` en `index.md`, conceptos y entidades para evitar que Obsidian resuelva por error hacia el archivo original en `raw/`.
- ✅ **Preserva explicaciones técnicas complejas:** Toda ingesta DEBE incluir la sección `## Detailed Breakdown` para conservar explicaciones de herramientas, subsecciones, niveles de diagnóstico y metodologías originales de `raw/`.
- ✅ **Preserva diagramas y código:** Si la fuente contiene bloques ` ```mermaid ` o pseudocódigo/código, debes incluirlos íntegramente en las secciones `## Diagrams & Visualizations` y `## Code & Pseudocode Examples`.
